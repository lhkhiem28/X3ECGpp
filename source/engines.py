...
def train_fn(
    ...
    for epoch in tqdm(range(1, num_epochs + 1), disable = training_verbose):
        ...
        for (ecgs, demogrs), r_counts, labels in tqdm(train_loaders["train"], disable = not training_verbose):
            (ecgs, demogrs), r_counts, labels = (ecgs.cuda(), demogrs.cuda()), r_counts.cuda(), labels.cuda()

            logits, sub_logits = model((ecgs, demogrs))
            loss, sub_loss = criterion(logits, labels), F.l1_loss(sub_logits, r_counts)

            (loss + 0.02*sub_loss).backward()
            optimizer.step(), optimizer.zero_grad()

            running_loss, running_sub_loss = running_loss + loss.item()*ecgs.size(0), running_sub_loss + loss.item()*ecgs.size(0)
        ...
        (epoch_loss, epoch_sub_loss), epoch_f1 = (running_loss/len(train_loaders["train"].dataset), running_sub_loss/len(train_loaders["train"].dataset)), f1_score(
            running_labels, running_preds
            , average = "macro"
        )
        if training_verbose:
        ...
        with torch.no_grad():
            ...
            running_loss = 0.0
            running_labels, running_preds = [], []
            for (ecgs, demogrs), r_counts, labels in tqdm(train_loaders["val"], disable = not training_verbose):
                (ecgs, demogrs), r_counts, labels = (ecgs.cuda(), demogrs.cuda()), r_counts.cuda(), labels.cuda()
